...
void Admin::show_menu() {
	std::cout << "\n======================================\n";
	std::cout << "ADMIN COMMAND CENTER | Dept: " << this->department << "\n";
	std::cout << "======================================\n";
	std::cout << "1. Add New Doctor Profile\n";
	std::cout << "2. Register New Patient Profile\n";
	std::cout << "3. Manually SUSPEND / LOCK a User Account\n"; // <--- New Choice Hook!
	std::cout << "4. CLEAR Account Lockout Overrides (Unlock)\n";
	std::cout << "5. Run System Master Database Audit\n";
	std::cout << "6. Exit / Logout from Session\n";
	std::cout << "======================================\n";
...
void Admin::add_doctor(Storage<Doctor>& dr_store) {
	...
	do {
		std::cout << "Enter Doctor ID to Add (Enter '0' to exit) : ";
		std::cin >> ID;
		ID = ID.trim();
		if (ID == "0") return;
...
void Admin::add_patient(Storage<Patient>& pat_store) {
	...
	do {
		std::cout << "Enter Patient ID to Add (Enter '0' to exit) : ";
		std::cin >> ID;
		ID = ID.trim();
		if (ID == "0") return;
...
void Admin::view_all_records(Storage<Doctor>& dr_store, Storage<Patient>& pat_store, Storage<Admin>& adm_store, Storage<Appointment>& appt_store) {
	...
	std::cout << "[3] REGISTERED PATIENTS\n";
	std::cout << "---------------------------------------------------\n";
	ct = pat_store.get_count();
	if (ct == 0) {
		std::cout << "No Patients registered in the System.\n";
	...
	std::cout << "[3] APPOINTMENTS SCHEDULED\n";
	std::cout << "---------------------------------------------------\n";
