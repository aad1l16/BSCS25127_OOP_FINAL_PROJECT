...
void Doctor::diagnose_patient(Storage<Appointment>& appt_store, Storage<Patient>& pat_store) {
	...
	String tempStr;
	do {
		std::cout << "\nEnter Appointment ID ( or type '0' to cancel ): ";
		std::cin >> tempStr;
		...
		if (!HospitalSystem::is_valid_id(tempStr, "APT_")) {
			...
			std::cout << "\nError: Invalid ID Format! Appointments must start with 'APT_'.\n";
			continue;
		}
		...
		if (tempAppt->get_doctor_id() != this->UserID) {
			...
			std::cout << "\nThis appointment does not belong to you. Try Again.\n";
			tempAppt = nullptr;
			continue;
		}
		if (tempAppt->get_status() == "Completed" || tempAppt->get_status() == "Cancelled") {
			...
			std::cout << "\nError: This appointment has already been " << tempAppt->get_status() << "!\n";
			tempAppt = nullptr;
			continue;
		}
